...
public class Color {
	...
	public void lerp(Color tmpCN, float alpha) {
	...
	public void tintForTorch(int f, int d, int l) {//l is night, 0 = day, 15= night
		//15 d = half tint
		//o d = full tint
		//f is how much tint
		if (f == 0) return;
		float dayAlpha = d / 15f;
		//dayAlpha /= 2f;
		//dayAlpha += .5f;
		float torchTint = (f-d+7)/ 15f;
		torchTint = MathUtils.clamp(torchTint,  0, 1);
		...
		float greenTint = 0;
		if (torchTint > 0){
			torchTint += MathUtils.random(-.12f, .12f);
			globalTint = MathUtils.random(.9f, 1f);
			greenTint = MathUtils.random(0.13f, .14f);
		}
		
		torchTint *= .4;
		
		greenTint *= torchTint;
		greenTint = 1-greenTint;
		
		
		//torchTint += .5f;
		torchTint = MathUtils.clamp(torchTint,  0, 1);
				//Math.max(0, torchTint);
	...
	public Color mulNoA (float value) {
		this.r *= value;
		this.g *= value;
		this.b *= value;
		//this.a *= value;
