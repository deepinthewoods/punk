...
import com.niz.punk.CorneredSprite;
...
public class TextureAtlas implements Disposable {
	...
	public CorneredSprite createCorneredSprite(String name) {
		for (int i = 0, n = regions.size; i < n; i++){
			
			if (regions.get(i).name.equals(name)) return newCorneredSprite(regions.get(i));
			
		}
		return null;
	}
	private CorneredSprite newCorneredSprite(AtlasRegion region) {
		if (region.packedWidth == region.originalWidth && region.packedHeight == region.originalHeight) {
			if (region.rotate) {
				CorneredSprite sprite = new CorneredSprite(region);
				sprite.setBounds(0, 0, region.getRegionHeight(), region.getRegionWidth());
				sprite.rotate90(true);
				return sprite;
			}
			return new CorneredSprite(region);
		}
		return new CorneredSprite(region);//AtlasSprite(region);
	}
