...
public class OgreAnimation extends LayeredAnimation {
	static int framecount = 61;
	static int layercount = 40;
	public OgreAnimation(TextureAtlas[] atlasses, int index) {
		
		super(framecount, layercount, index);
		
		TextureAtlas atlas = atlasses[index];

		for (int i = 0; i < framecount; i++){
			
			float xx=4f, yy = 3.5f+.0625f*2;
			layers[0][i] = new CorneredSprite(atlas.findRegion("ogre", i), xx,yy);
			
			layers[33][i] = new CorneredSprite(atlas.findRegion("ogrehgl", i), xx,yy);
			layers[34][i] = new CorneredSprite(atlas.findRegion("ogrehgr", i), xx,yy);
			layers[35][i] = new CorneredSprite(atlas.findRegion("ogrewtr", i), xx,yy);
			...
			//layers[38][i] = new CorneredSprite(atlas.findRegion("ogreorigin", i), xx,yy);
			layers[39][i] = new CorneredSprite(atlas.findRegion("ogrewtl", i), xx,yy);
	...
	private StateFrameInfo[] initStateAnims() {
		...
		states[0] = new StateFrameInfo(14,5, true, 1f){
			public void newFrame(GenericMob genericMob, PunkMap map, World world) {	} };
			
		states[31] = new StateFrameInfo(44,7, false, 3f){//die
				public void newFrame(GenericMob genericMob, PunkMap map, World world) {	} };
			
		states[1] = new StateFrameInfo(1,11, true, .11f){//walk
			public void newFrame(GenericMob genericMob, PunkMap map, World world) {	} };;
			
		states[2] = new StateFrameInfo(1,11, true,  .08f){//run
			public void newFrame(GenericMob genericMob, PunkMap map, World world) {	} };;
			
		states[3] = new StateFrameInfo(12,11, true,  .1f){//
			public void newFrame(GenericMob genericMob, PunkMap map, World world) {	} };;
			
		states[4] = new StateFrameInfo(11,1, true,  .2f){//jump1
			public void newFrame(GenericMob genericMob, PunkMap map, World world) {	} };;
		states[5] = new StateFrameInfo(11,1,false,  .2f){//jump 2
			public void newFrame(GenericMob genericMob, PunkMap map, World world) {	} };;
		states[10] = new StateFrameInfo(11,1,true,  .2f){//fall
			public void newFrame(GenericMob genericMob, PunkMap map, World world) {	} };;
		...
		states[30] = new StateFrameInfo(51, 10, true,  .4f){
			public void newFrame(GenericMob genericMob, PunkMap map, World world) {	} };;
