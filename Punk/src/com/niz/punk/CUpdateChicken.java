...
public class CUpdateChicken extends ComponentUpdate {
	...
	public void passiveTargetAcquisition(PunkMap map, GenericMob mob){
		...
		//if (mob.peers.size <2){
			...
			loc.set(mob.position.tmp().add(MathUtils.random(-30,30), MathUtils.random(-30,5)));
			int li = map.getBlock(loc).getLight();
			if ((li > 12 || map.getBlock(mob.targetB).getLight() == 0)){
				mob.targetB.set(loc);
