...
public class SSSize implements SpawnStrategy {
...
public MobInfo mob;
	...
	public void attemptSpawn(PunkMap map, Player player, World world, PunkBodies monsterIndex) {
		...
		if (map.chunkActors.mobPool[0].monsterList.size >= size) return;
		...
		if (map.getBlock(x,y+1).blockType() ==0 && map.getBlock(x,y+2).blockType() == 0){
			...
			//TODO map.chunkActors.mobPool[0].createMonster(x,y, mob, PunkBodies.factions[92], world, monsterIndex, player, map);
			updateTotal++;
		}
