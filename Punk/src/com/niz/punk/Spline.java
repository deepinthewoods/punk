...
public class Spline {
    ...
    public Spline(float[] xx, float[] yy) {
    	this.xx = new float [xx.length];
    	this.yy = new float [yy.length];
    	if (xx.length != yy.length)throw new GdxRuntimeException("spline values don't line up");
    	for (int i = 0; i < xx.length; i++){
    		this.xx[i] = xx[i];
    		this.yy[i] = yy[i];
    		//Gdx.app.log("spline", "add node "+xx[i]+","+yy[i]);
    	}
        setValues();
    }
     ...
     */
    public float getValuen(float x) {
        if (xx.length == 0) {
            return 0;
        }
        ...
        //    return (float)yy[0];
        //}
        if (index == xx.length-1)return yy[index];
        float alpha = xx[index] - xx[index+1];
        alpha = (xx[index]-x)/alpha;
        return Chunk.lerp(yy[index],yy[index+1],alpha);
        //return (float)(a[index] + b[index] * (x - xx[index]) + c[index]
    ...
    public float getValue(float x) {
        ...
        float value = x - xx[storageIndex];
        return (float)(a[storageIndex] + b[storageIndex] * value
